    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-green-100 hover:shadow-2xl transition-all duration-300 p-5">
        <h2 className="text-lg font-semibold mb-4 flex items-center text-gray-800">
          <FiUsers className="mr-2 text-blue-600" /> Drivers ({drivers.length})
        </h2>
      ...
      <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-blue-100 hover:shadow-2xl transition-all duration-300 p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <InfoCard label="Vehicle" value={selected.vehicle} />
          <InfoCard label="Current Location" value={selected.location} />
          <InfoCard label="Completed" value={`${selected.completed} / ${selected.total}`} />
        </div>

        <div className="mb-4">
          <h4 className="font-semibold text-gray-700 mb-2">Current Route</h4>
          <p className="text-gray-600">{selected.routeStops} stops remaining</p>
        </div>

        {/* Progress Bar */}
        <div>
          <h4 className="font-semibold text-gray-700 mb-2">Delivery Progress</h4>
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div
              className="bg-gradient-to-r from-blue-600 to-green-500 h-3 rounded-full transition-all"
              style={{
                width: `${(selected.completed / selected.total) * 100}%`,
              }}
            ></div>
          </div>
